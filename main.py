# here we gonna make the code of all pkls here and in app.py we gonna make our streamlit code
import os
import pickle
import pandas as pd
import httpx
...
if not TMDB_API_KEY:
    raise ValueError("No TMDB API key found. Please set the TMDB-API-KEY environment variable.")

@asynccontextmanager
async def lifespan(app: FastAPI):
    # Load pickles on startup
...
app = FastAPI(title="Movie Recommender System API", version="1.0", lifespan=lifespan)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
